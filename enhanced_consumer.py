"""
Enhanced Fraud Detection Consumer with Streaming Context
"""
import json
...
class EnhancedFraudDetectionConsumer:
    """Consumes transactions and performs streaming-enhanced fraud detection"""
    
    def __init__(self):
        """Initialize Kafka consumer, producer, and enhanced AI agents"""
        ...
        # Enhanced AI Agent Coordinator with Streaming Context
        ...
        self.collaboration_triggered = 0
    
    def process_transaction(self, transaction_data: dict):
        """
        Process a transaction with streaming-enhanced multi-agent analysis
        ...
        try:
            ...
            # Analyze with streaming-enhanced multi-agent system
            decision = self.agent_coordinator.analyze_transaction(transaction)
            
            # Track velocity alerts
            context_stats = self.agent_coordinator.get_context_statistics()
    ...
    def print_statistics(self):
        """Print processing statistics with streaming context"""
        context_stats = self.agent_coordinator.get_context_statistics()
        
        print(f"\n{Fore.CYAN}{'='*80}")
        print(f"{Fore.CYAN}📊 STATISTICS (Streaming-Enhanced)")
        ...
        print(f"\n{Fore.MAGENTA}Streaming Context:{Style.RESET_ALL}")
        print(f"  Active Customers: {context_stats['active_customers']}")
        print(f"  Total Customers: {context_stats['total_customers']}")
        print(f"{Fore.CYAN}{'='*80}{Style.RESET_ALL}\n")
    
    def run(self):
        """Run the enhanced consumer"""
        print(f"\n{Fore.CYAN}{'='*80}")
        print(f"{Fore.CYAN}🤖 ENHANCED FRAUD DETECTION CONSUMER STARTED")
        print(f"{Fore.CYAN}Consuming from: {config.KAFKA_TOPIC_TRANSACTIONS}")
        print(f"{Fore.CYAN}Streaming Context Intelligence: ACTIVE")
        print(f"{Fore.CYAN}Adaptive Agent Collaboration: ENABLED")
        print(f"{Fore.CYAN}{'='*80}{Style.RESET_ALL}\n")
        ...
        try:
            for message in self.consumer:
                ...
                # Print statistics every 5 transactions
